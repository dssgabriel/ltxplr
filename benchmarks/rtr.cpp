...
{
  using layout_rtr = ltxplr::rtr<TExts...>::template mapping<Exts>;
  layout_rtr map{};
  ...
  fmt::println("to_off_eps ({}) = {}\n", idx_arr, map.tile_outer_offset_fold_prep_strides(idxs...));
  ...
  fmt::println("ti_off_eps ({}) = {}\n", idx_arr, map.tile_inner_offset_fold_prep_strides(idxs...));

  fmt::println("toff_e     ({}) = {}\n", idx_arr, map.tiled_offset_fold_direct(idxs...));

  fmt::println("off_l      ({}) = {}", idx_arr, map.offset_for(idxs...));
  fmt::println("off_e      ({}) = {}\n", idx_arr, map.offset_fold(idxs...));
}
  ...
  requires (2 == Exts::rank() and 2 == sizeof...(TExts) and 2 == sizeof...(I))
{
  ...
  b2.run("rtr_offset for direct", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_for_direct(idxs...) + map.tile_inner_offset_for_direct(idxs...)); });
  b2.run("rtr_offset for direct strides", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_for_direct_strides(idxs...) + map.tile_inner_offset_for_direct_strides(idxs...)); });
  b2.run("rtr_offset for prep", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_for_prep(idxs...) + map.tile_inner_offset_for_prep(idxs...)); });
  b2.run("rtr_offset for prep strides", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_for_prep_strides(idxs...) + map.tile_inner_offset_for_prep_strides(idxs...)); });
  b2.run("rtr_offset fold direct", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_fold_direct(idxs...) + map.tile_inner_offset_fold_direct(idxs...)); });
  b2.run("rtr_offset fold direct strides", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_fold_direct_strides(idxs...) + map.tile_inner_offset_fold_direct_strides(idxs...)); });
  b2.run("rtr_offset fold prep", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_fold_prep(idxs...) + map.tile_inner_offset_fold_prep(idxs...)); });
  b2.run("rtr_offset fold prep strides", [&]() { nb::doNotOptimizeAway(map.tile_outer_offset_fold_prep_strides(idxs...) + map.tile_inner_offset_fold_prep_strides(idxs...)); });
  b2.run("tiled_offset fold direct", [&]() { nb::doNotOptimizeAway(map.tiled_offset_fold_direct(idxs...)); });
  ...
  b3.run("rtr_offset for direct strides", [&]() {
    auto tmp = 0UL;
    for (size_t i = 0; i < ext.extent(0); ++i) {
      for (size_t j = 0; j < ext.extent(1); ++j) {
  ...
  b3.run("rtr_offset for prep strides", [&]() {
    auto tmp = 0UL;
    for (size_t i = 0; i < ext.extent(0); ++i) {
      for (size_t j = 0; j < ext.extent(1); ++j) {
  ...
  b3.run("rtr_offset fold direct strides", [&]() {
    auto tmp = 0UL;
    for (size_t i = 0; i < ext.extent(0); ++i) {
      for (size_t j = 0; j < ext.extent(1); ++j) {
  ...
  b3.run("rtr_offset fold prep strides", [&]() {
    auto tmp = 0UL;
    for (size_t i = 0; i < ext.extent(0); ++i) {
      for (size_t j = 0; j < ext.extent(1); ++j) {
  ...
  b4.run("rtr_offset for direct strides", [&]() {
    auto tmp = 0UL;
    for (size_t x = 0; x < ext.extent(0); x += TExts...[0]) {
      for (size_t y = 0; y < ext.extent(1); y += TExts...[1]) {
  ...
  b4.run("rtr_offset for prep strides", [&]() {
    auto tmp = 0UL;
    for (size_t x = 0; x < ext.extent(0); x += TExts...[0]) {
      for (size_t y = 0; y < ext.extent(1); y += TExts...[1]) {
  ...
  b4.run("rtr_offset fold direct strides", [&]() {
    auto tmp = 0UL;
    for (size_t x = 0; x < ext.extent(0); x += TExts...[0]) {
      for (size_t y = 0; y < ext.extent(1); y += TExts...[1]) {
  ...
  b4.run("rtr_offset fold prep strides", [&]() {
    auto tmp = 0UL;
    for (size_t x = 0; x < ext.extent(0); x += TExts...[0]) {
      for (size_t y = 0; y < ext.extent(1); y += TExts...[1]) {
...
auto main(int argc, char* argv[]) -> int {
  ...
  case 4:
    ...
    check<exts4, 10, 10, 10, 10>(16, 32, 64, 8);
